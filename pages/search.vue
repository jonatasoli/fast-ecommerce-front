<script setup lang="ts">
  ...
  const { data, pending, refresh } = await useFetch<PaginatedProducts>(url, {
    watch: [page],
    query: {
      search: query,
      page,
...
<template>
  <main class="search container">
    ...
    <div v-if="pending" class="search__skeleton">
      <div v-for="n in OFFSET" :key="n">
        <ProductSkeleton />
      </div>
    </div>
    <div v-else-if="data?.products.length === 0" class="search__empty">
      <n-alert type="info" :title="t('search.empty')" bordered />
    </div>
    <div v-else class="search__products">
      <div v-for="product in products" :key="product.product_id">
        <ProductCard :product="product" @add-to-cart="handleAddToCart" />
      </div>
    </div>
    <n-space align="flex-end" vertical class="search__pagination">
