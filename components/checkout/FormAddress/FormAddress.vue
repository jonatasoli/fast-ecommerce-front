<script lang="ts" setup>
  ...
  const validationSchema = toTypedSchema(
    zod.object({
      ...
      neighborhood: zod
        .string()
        .min(3, t('checkout.shipping.form.neighborhoodRequired')),
      street: zod.string().min(3, t('checkout.shipping.form.streetRequired')),
      street_number: zod
        .string()
        .min(1, t('checkout.shipping.form.numberRequired')),
  ...
  const addressComplement = defineComponentBinds(
    'address_complement',
    naiveConfig,
  )
  ...
  function handleGetAddressByZipcode() {
    ...
    cartStore
      .getAddressByZipcode(zipcode.value.value, props.addresType)
      .then(() => {
